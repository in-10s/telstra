$(function () {
    ...
    var mapChart=$('#map_chart').highcharts('Map', {
            	...
            	point:{
                	events:{
                    	click: function(){
							...
							if(this.name=='Al Khor'){
							//alert('map1 ');
								map1(this.name,Number(this.value).toFixed(3));
							}else if(this.name=='Al Rayyan'){
							//alert('map2');
								map2(this.name,this.value);
							}else if(this.name=='Doha'){
							//alert('map3');
        ...
        series : [{
            ...
            name: 'Separators',
            type: 'mapline',
            data: Highcharts.geojson(Highcharts.maps['countries/qa/qa-all'], 'mapline'),
            color: 'silver',
            showInLegend: false,
	...
	$('#map_chart_drill').highcharts({title : {text : ''},
        ...
        series: [{name: 'Service',colorByPoint: true,
            data: [{name: 'Mobility',y: 4150.00,drilldown: 'Mobility',color: '#FF9800'}, 
					...
					{name: 'Cloud Solutions',y: 2732.00,color: '#2196F3'}]
        }],		
        drilldown: {
            series: [{
                name: 'Mobility',
	...
	$('#map_chart_drill').highcharts({title : {text : ''},
        ...
        series: [{name: serviceName,colorByPoint: true,
            data: [{name: 'Mobility',y: 1037.50,drilldown: 'Mobility',color: '#019E0F'}, 
					...
					{name: 'Cloud Solutions',y: 956.20,color: '#CD0B02'}]
        }],		
        drilldown: {
	...
	$('#map_chart_drill').highcharts({title : {text : ''},
        ...
        series: [{name:serviceName,colorByPoint: true,
            data: [{name: 'Mobility',y: 1452.50,drilldown: 'Mobility',color: '#2196F3'}, 
					...
					{name: 'Cloud Solutions',y: 683.00,color: '#FF9800'}]
        }],		
        drilldown: {
            series: [{tooltip: {
            headerFormat: '<span style="font-size:11px">{series.name}</span><br>',
	...
	$('#map_chart_drill').highcharts({title : {text : ''},
        ...
        series: [{name:serviceName,colorByPoint: true,
            data: [{name: 'Mobility',y: 1660.00,drilldown: 'Mobility',color: '#50B432'}, 
					...
					{name: 'Cloud Solutions',y: 1092.80,color: '#DDDF00'}]
        }],		
        drilldown: {
            series: [{
				tooltip: {
