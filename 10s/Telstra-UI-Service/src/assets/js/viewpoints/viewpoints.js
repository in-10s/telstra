...
//    //    reqData.U_ID = userName;
    ...
    procesRequest("addaccounts.action", reqData, fnExternalUsersAccountSucc, fnExternalUsersFail, false);



}
var service_grid = "";
function addservicesCodePopUp() {
    // alert("sdfsd")
    debugger;
    if ($("#service"))
        $("#service").val("");
...
function createCostCenter() {
    ...
    try {
        ...
        reqData.LEVEL = 0;
        $('#costCenterDiv').val('');
        procesRequest("createViewPointNode.action", reqData, costCenterCreateSuccess, costCenterCreateFail, false);
    } catch (e) {
        alert(e)
    }
}
...
function costCenterCreateSuccess(resp) {
    ...
    if (resp.objCRSResponse.success) {
        ...
        // reqData.COMPANY_ID = companyId;
        reqData.PARENT_ID = -1;
        procesRequest("reloadVPData.action", reqData, relaodCostCenterSucess, relaodCostCenterFail, false);
...
//    }
    ...
    procesRequest("insertAccounts.action", reqData, fnsaveAccsSucc, fnsaveAccsFail, false);
...
function fnDeleteAccount(item) {
    ...
    procesRequest("deleteAccount.action", reqParams, deleteAccountSuccess, deleteAccountFail, false);
}
...
function loadAccountsGrid() {
    ...
    var reqParams = {};
    procesRequest("loadViewPointtabledata.action", reqParams, loadAccountsGridSuccess, loadAccountsGridFail, false);
}
...
//        });
        ...
        if (finalCheckRow.length > 0) {
            ...
            procesRequest("deleteAccount.action", reqData, deleteAccountSuccess, deleteAccountFail, false);
//        reqData.PARENT_CENTER_ID = selectedNode.CC_ID;
            //alert(JSON.stringify(reqData))
...
//        });
        ...
        if (finalCheckRow.length > 0) {
            ...
            procesRequest("deleteNodeAccount.action", reqData, deleteNodeAccountSuccess, deleteNodeAccountFail, false);
//        reqData.PARENT_CENTER_ID = selectedNode.CC_ID;
            //alert(JSON.stringify(reqData))
            //procesRequest("addAssignToCCenter.action", reqData, addAssignSuccess, addAccountsFail);
        } else {
...
//    alert("this node delete = " + selectedNode.CC_ID)
    procesRequest("deleteNodeHierarchy.action", reqData, deleteNodeSuccess, NodedeleteFail, false);
}
...
function onSelectedNode(nodeId) {
    ...
    procesRequest("fetchSelectedNodeAccounts.action", reqData, NodeAccountsSuccess, fetchSelectedNodeAccountsFail, false);
}
