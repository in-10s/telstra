...
//            alert("value :" + $(this).val());
            ...
            if (userTypeValue == "Internal User") {
                if (internalUserList.length == 0) {
                    $('#user_accss_per_div').css("display", "none");
                    $('#user_accss_per_save').css("display", "none");
                    $('#user_accss_per_div_msg').css("display", "inline");
                } else {
                    $('#user_accss_per_div').css("display", "inline");
                    $('#user_accss_per_div_msg').css("display", "none");
                    $('#user_accss_per_save').css("display", "inline");
                    $("#userListView").data("kendoListView").dataSource.data(internalUserList);
                }
            } else {
...
//        alert("LIST :" + externalUserList.toSource());
...
//        template: "<div style='line-height: 25px;'>#:COMPANY_NAME#</div>"
            template: "<div><li class='act' style='line-height: 25px;'><div ><input name='company_list' onclick='onClickOnCustomerCombo(#:COMPANY_ID#);' id='#:COMPANY_NAME#' company_id='#:COMPANY_ID#' type='checkbox'  class='chkBoxCls company_checkbox' value='#:COMPANY_NAME#'><label class='textoverflowec' id='#:COMPANY_NAME#' title='#:COMPANY_NAME#'>#:COMPANY_NAME#</label></div> </li></div>"
...
//        $("#costCenterList_NoRecordFound_Div").show();
        ...
        if (userTypeValue == "Internal User") {
            for (var index = 0; savedCompanies.length > index; index++) {
                $("input[name=company_list][company_id='" + savedCompanies[index] + "']").prop("checked", true);
            }
        } else {
...
//    }
    ...
    var userTypeValue = $('input[type=radio][name="usertype"]:checked').val();
    if (userTypeValue == "Internal User") {
        $("input[name=company_list][company_id='" + companyID + "']").prop("checked", true);
    } else {
...
function saveUserPermissionDetails() {
    ...
    var userTypeValue = $('input[type=radio][name="usertype"]:checked').val();
    if (userTypeValue == "Internal User") {
        ReqData.USER_TYPE = "internal";
        ReqData.UNCHECK_COMPANIES = strUnchekCompanies;
    } else {
...
//            },
    ...
    $("#account_list_grid .k-grid-content").css('height', '300');
}
