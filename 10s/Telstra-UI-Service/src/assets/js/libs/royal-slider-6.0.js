...
(function($) {
	...
	function RoyalSlider(element, options) {
		...
		this.slides.each(function() {			
			...
			if(self.settings.blockLinksOnDrag) {
				var clickEvent = this.hasTouch ? 'touchend.royalslider' : 'click.royalslider';
				jqSlide.find('a').bind(clickEvent, function(e) {					
		...
		if(this.currentSlideId != 0) {
			if(!this._useWebkitTransition) {
				...
				this._dragContainer.css({ '-webkit-transition-duration':'0ms', '-webkit-transition-property': 'none' });
		...
		if(this.settings.welcomeScreenEnabled) {
			...
			function hideWelcomeScreen(preloadNearby) {	
				...
				var delay=self.settings.welcomeScreenShowSpeed + 100;
				//setTimeout(function(){ self._startSlider(); }, delay);
				var ts=setTimeout(function(){ self._startSlider(); }, delay);		
				if(ts<=100)
				    self._startSlider();
	...
	RoyalSlider.prototype = {
			 ...
			 */			 
			goTo:function(id, silent, fromNav) {	
				if(!this.isAnimating) {
					this.isAnimating = true;
					var self = this;
			...
			updateSliderSize:function() {
				var self = this;
				/*this.sliderWidth = this._slidesWrapper.width()  ;
				this.sliderHeight = this._slidesWrapper.height() ;*/
				...
				if(this.sliderWidth != this.slideWidth || this.sliderHeight != this.slideHeight) {
					...
					for ( var i=0, len=arLen; i<len; ++i ){
						...
						if(this.settings.slideSpacing > 0 && i < arLen - 1) {
							_currItem.slide.css({'margin-right': this.settings.slideSpacing});
						}
			...
			_onDragRelease:function(e) {		
				...
				if(this._isDragging) {			
					...
					if(this.endPos == this._startPos) {
						/*if(this.settings.nonDraggableClassEnabled && $(e.target).hasClass('non-draggable')) {
	...
	$.fn.royalSlider.defaults = {    
			...
			nonDraggableClassEnabled:false,          // Prevents dragging on all elements that have 'non-draggable' class inside slide.
